...
namespace BSWrapperObjects.Internal.UnityWrapper
{
    ...
    public class BSWrapperObject_UnityEngine_Tilemaps_TilemapCollider2D : BSWrapperObject < TilemapCollider2D >

    {
        #region Public

        public BSWrapperObject_UnityEngine_Tilemaps_TilemapCollider2D( TilemapCollider2D obj ) : base( obj )
        {
            m_Properties["maximumTileChangeCount"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.maximumTileChangeCount ),
                x => m_InternalObject.maximumTileChangeCount = WrapperHelper.UnwrapObject < uint >( x ) );

            m_Properties["extrusionFactor"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.extrusionFactor ),
                x => m_InternalObject.extrusionFactor = WrapperHelper.UnwrapObject < float >( x ) );

            m_Properties["hasTilemapChanges"] = new BSReflectionReference(
                () => m_InternalObject.hasTilemapChanges ? BSObject.One : BSObject.Zero,
                null );
