...
namespace BSWrapperObjects.Internal.UnityWrapper
{
    ...
    public class BSWrapperObject_UnityEngine_AreaEffector2D : BSWrapperObject < AreaEffector2D >

    {
        #region Public

        public BSWrapperObject_UnityEngine_AreaEffector2D( AreaEffector2D obj ) : base( obj )
        {
            m_Properties["forceAngle"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.forceAngle ),
                x => m_InternalObject.forceAngle = WrapperHelper.UnwrapObject < float >( x ) );

            m_Properties["useGlobalAngle"] = new BSReflectionReference(
                () => m_InternalObject.useGlobalAngle ? BSObject.One : BSObject.Zero,
                x => m_InternalObject.useGlobalAngle = WrapperHelper.UnwrapObject < bool >( x ) );
