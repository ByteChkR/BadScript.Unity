...
namespace BSWrapperObjects.Internal.UnityWrapper
{
    ...
    public class BSWrapperObject_UnityEngine_UI_CanvasScaler : BSWrapperObject < CanvasScaler >

    {
        #region Public

        public BSWrapperObject_UnityEngine_UI_CanvasScaler( CanvasScaler obj ) : base( obj )
        {
            ...
            m_Properties["referenceResolution"] = new BSReflectionReference(
                () => new BSWrapperObject_UnityEngine_Vector2( m_InternalObject.referenceResolution ),
                x => m_InternalObject.referenceResolution = WrapperHelper.UnwrapObject < Vector2 >( x ) );

            m_Properties["matchWidthOrHeight"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.matchWidthOrHeight ),
                x => m_InternalObject.matchWidthOrHeight = WrapperHelper.UnwrapObject < float >( x ) );

            m_Properties["fallbackScreenDPI"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.fallbackScreenDPI ),
                x => m_InternalObject.fallbackScreenDPI = WrapperHelper.UnwrapObject < float >( x ) );

            m_Properties["defaultSpriteDPI"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.defaultSpriteDPI ),
                x => m_InternalObject.defaultSpriteDPI = WrapperHelper.UnwrapObject < float >( x ) );

            m_Properties["dynamicPixelsPerUnit"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.dynamicPixelsPerUnit ),
                x => m_InternalObject.dynamicPixelsPerUnit = WrapperHelper.UnwrapObject < float >( x ) );
