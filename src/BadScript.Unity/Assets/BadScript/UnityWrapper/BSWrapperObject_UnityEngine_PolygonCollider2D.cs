...
namespace BSWrapperObjects.Internal.UnityWrapper
{
    ...
    public class BSWrapperObject_UnityEngine_PolygonCollider2D : BSWrapperObject < PolygonCollider2D >

    {
        #region Public

        public BSWrapperObject_UnityEngine_PolygonCollider2D( PolygonCollider2D obj ) : base( obj )
        {
            m_Properties["autoTiling"] = new BSReflectionReference(
                () => m_InternalObject.autoTiling ? BSObject.One : BSObject.Zero,
                x => m_InternalObject.autoTiling = WrapperHelper.UnwrapObject < bool >( x ) );

            m_Properties["pathCount"] = new BSReflectionReference(
                () => new BSObject( ( decimal ) m_InternalObject.pathCount ),
                x => m_InternalObject.pathCount = WrapperHelper.UnwrapObject < int >( x ) );
